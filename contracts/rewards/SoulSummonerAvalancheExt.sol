...
// summoner of souls | ownership transferred to a governance smart contract 
// upon sufficient distribution + the community's desire to self-govern.
// Enchantress: @0xBuns

contract SoulSummonerAvalancheExt is AccessControl, ReentrancyGuard {
    ...
    function togglePause(bool enabled) external obey(isis) {
        isPaused = enabled;
    }
    ...
    function addPool(uint _allocPoint, IERC20 _lpToken, bool _withUpdate, uint _feeDays) public isSummoned obey(isis) { 
    ...
    function set(uint pid, uint allocPoint, bool withUpdate) external isSummoned validatePoolByPid(pid) obey(maat) {
    ...
    // view: pending soul rewards (for sender)
    function pendingSoul(uint pid) public view returns (uint pendingAmount) {
        Pools memory pool = poolInfo[pid];
        Users memory user = userInfo[pid][msg.sender];
    ...
    // calculates: user portion of total deposits for a given pid.
    function userShare(uint pid, address account) external view returns (uint share) {
        Pools memory pool = poolInfo[pid];
        Users memory user = userInfo[pid][account];

        // gets: total supply of lpToken in summoner (pool)
        uint totalDeposited = pool.lpToken.balanceOf(address(this));
        // gets: total staked (user)
        uint userStaked = user.amount;
        // calculates: user share of totalDeposited
        share = userStaked / totalDeposited;

        return share;
    }

    // gets: price for a given pid.
    function getPrice(uint pid) public view returns (uint tokenPrice) {
        Pools memory pool = poolInfo[pid];
        address tokenAddress = address(pool.lpToken);
        // [if] staking pool, [then] use soulPrice
        if (pid == 0) { tokenPrice = calculator.getSoulPrice(); }
        // [else] use lpPrice
        else { tokenPrice = calculator.getPoolPrice(tokenAddress); }
        return tokenPrice;
    }

    // gets: deposited value (pid, account)
    function getDepositedValue(uint pid, address account) external view returns (uint depositValue) {
        Users memory user = userInfo[pid][account];

        uint price = getPrice(pid);
        uint depositAmount = user.amount;

        depositValue = price * depositAmount;

        return depositValue;
    }

    // gets: deposited value (pid)
    function getPoolTVL(uint pid) public view returns (uint poolTVL) {
        Pools memory pool = poolInfo[pid];
        uint totalDeposited = pool.lpToken.balanceOf(address(this));
        uint price = getPrice(pid);

        poolTVL = price * totalDeposited;

        return poolTVL;
    }

    // gets: total value deposited
    function getTVL(uint[] calldata _pids) external view returns (uint valueLocked) {
            for (uint i = 0; i < _pids.length; ++i) {
            valueLocked += getPoolTVL(_pids[i]);
        }
    }

    // calculates: pending value (pid, account)
    function getPendingValue(uint pid, address account) external view returns (uint pendingValue) {
        uint pendingRewards = pendingSoul(pid, account);
        uint soulPrice = calculator.getSoulPrice();
        pendingValue = pendingRewards * soulPrice;

        return pendingValue;
    }

    // deposit: lp tokens (lp owner)
    function deposit(uint pid, uint amount) public nonReentrant isSummoned isActive validatePoolByPid(pid) {
    ...
    function withdraw(uint pid, uint amount) external nonReentrant isSummoned isActive validatePoolByPid(pid) {
    ...
    function enterStaking(uint amount) public nonReentrant isSummoned {
    ...
    function leaveStaking(uint amount) external nonReentrant isSummoned {
        Pools storage pool = poolInfo[0];
        Users storage user = userInfo[0][msg.sender];
    ...
    // updates: calculator address (strictly used for frontend)
    function updateCalculator(address _calculator) external obey(isis) {
