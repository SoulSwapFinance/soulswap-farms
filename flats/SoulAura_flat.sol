...
contract SoulAura is Ownable {
    ...
    IBond bond = IBond(0x45D674014Ba240Fc5B8e2605b36DF6A8e470e78f);
    ...
    ISoulSwapPair soulUsdc = ISoulSwapPair(0x5cED9D6B44a1F7C927AF31A8Af26dEF60C776712);
    ...
    // gets: total voting power = twice the reserves in the LPs + soul supply + seance supply.
    function totalSupply() public view returns (uint) {

        // SOUL | SOUL-FTM + SOUL-USDC
        (uint totalSoulFtm, , ) = soulNative.getReserves(); // token0 = SOUL
        (,uint totalSoulUsdc, ) = soulUsdc.getReserves(); // token1 = SOUL
    ...
    function swapBondPower(address member) public view returns (uint raw, uint formatted) {
        uint TWO = bond.pendingSoul(2, member); // FTM-USDC
        uint THREE = bond.pendingSoul(3, member); // BTC-FTM
        uint FOUR = bond.pendingSoul(4, member); // BTC-USDC
        uint FIVE = bond.pendingSoul(5, member); // ETH-FTM
        uint SIX = bond.pendingSoul(6, member); // ETH-USDC
        uint SEVEN = bond.pendingSoul(7, member); // USDC-USDC

        uint power = TWO + THREE + FOUR + FIVE + SIX + SEVEN;

        return (power, fromWei(power));
    }

    // gets: member's (total) bond power
    function bondPower(address member) public view returns (uint raw, uint formatted) {
        (uint SoulBondPower, ) = soulBondPower(member);
        (uint SwapBondPower, ) = swapBondPower(member);

        uint power = SoulBondPower + SwapBondPower;
